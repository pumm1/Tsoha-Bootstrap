# Tietokantasovelluksen esittelysivu

## Työn aihe

Työn aiheenani on online keittokirja, jonne käyttäjät voivat lisätä omia reseptejään ja selata muiden resetepjä. Reseptien selaaminen ei vaadi käyttäjätunnuksia, mutta niiden lisääminen (kuten myös muokkaaminen ja poistaminen, kunhan käyttäjä on sama, joka myös sen sinne lisäsi!) vaatii rekisteröitymisen ja sisäänkirjautumisen.

## Yleisiä linkkejä:

* [Linkki sovellukseeni](http://sakureme.users.cs.helsinki.fi/tsoha/etusivu)
* [Linkki dokumentaatiooni](https://github.com/pumm1/Tsoha-Bootstrap/tree/master/doc/dokumentaatio.pdf)

* [Etusivu](http://sakureme.users.cs.helsinki.fi/tsoha/etusivu)
* [Kategoria: Ruoka](http://sakureme.users.cs.helsinki.fi/tsoha/1)
* [Kategoria: Juoma](http://sakureme.users.cs.helsinki.fi/tsoha/2)
* [login](http://sakureme.users.cs.helsinki.fi/tsoha/login)
* [register](http://sakureme.users.cs.helsinki.fi/tsoha/register)

